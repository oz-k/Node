function categoryChange(e) {
    var gugun = [["강남구","강동구","강북구","강서구","관악구","광진구","구로구","금천구","노원구","도봉구","동대문구","동작구","마포구","서대문구","서초구","성동구","성북구","송파구","양천구","영등포구","용산구","은평구","종로구","중구","중랑구"],
    ...
    for(x in sido) {
        if(e.value == sido[x]) {var d = gugun[x]; break;}
        if(x == 15) var d = ["군/구 선택"];
    }
...
var sido = [["서울특별시"],["인천광역시"],["대전광역시"],["광주광역시"],
            ["대구광역시"],["울산광역시"],["부산광역시"],["경기도"],
            ["강원도"],["충청북도"],["충청남도"],["전라북도"],
            ["전라남도"],["경상북도"],["경상남도"],["제주도"]]
select = document.getElementById('select1');
var opt = document.createElement('option');
opt.innerHTML = ["시/도 선택"];
select.appendChild(opt);
for(x in sido) {
    var opt = document.createElement('option');
    opt.value = sido[x];
    opt.innerHTML = sido[x];
    select.appendChild(opt);
}